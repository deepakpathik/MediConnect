...
export default function AppointmentScanner({ onScanComplete, theme }) {
  ...
  const extractAppointmentData = (text) => {
    ...
    // Look for specialty
    const specialtyPattern = /(Cardiologist|Dentist|Dermatologist|General Practitioner|Pediatrician|Neurologist)/i;
    const specialtyMatch = text.match(specialtyPattern);
    const specialty = specialtyMatch ? specialtyMatch[0] : '';
    ...
    return { date, time, doctorName, specialty, phone, notes };
  };
